...
GRIPPER_TCP_PRIM = "/World/gantry_home/ur10_flattened/ur10_instanceable/gripper_tcp"
...
# URDF / YAML for Lula IK — resolved from ISAACSIM_PATH set in .env or exported in the shell.
_isaacsim = os.environ.get("ISAACSIM_PATH", "")
URDF_PATH = os.path.join(_isaacsim, "exts", "isaacsim.asset.importer.urdf", "data", "urdf", "robots", "ur10", "urdf", "ur10.urdf") if _isaacsim else ""
YAML_PATH = os.path.join(_isaacsim, "exts", "isaacsim.robot_motion.motion_generation", "motion_policy_configs", "universal_robots", "ur10", "rmpflow", "ur10_robot_description.yaml") if _isaacsim else ""
...
GRIP_OVERSHOOT = 0.080   # 80mm extra closing past part width for firm contact
ENTRY_CLEARANCE = 0.020  # 20mm extra gap over part width for bin-entry pre-shape

# Gripper geometry
# GRIPPER_TCP_OFFSET is used ONLY for the URDF virtual-link joint definition
# (static scene geometry).  All runtime IK correction is derived from live
# prim measurements stored in STATE.calibrated_tcp_{x,y,z}_offset — do NOT
# use this constant in any IK target computation.
GRIPPER_TCP_OFFSET   = 0.214   # URDF virtual link geometry only
...
class BridgeState:
    def __init__(self):
        ...
        self.tcp_ee_offset = np.array([0.0, 0.0, 0.0])
        # Calibrated TCP→EE world offsets — all three axes populated from live
        # prim measurements.  Z is seeded from the init-time prim norm
        # (see _init_lula_ik); X/Y start at 0 and update from hover.
        self.calibrated_tcp_x_offset = 0.0
        self.calibrated_tcp_y_offset = 0.0
        self.calibrated_tcp_z_offset = 0.0   # seeded from live prim at IK init
...
async def _approach_target(params):
    ...
    * ``raw_z=False`` (default, pick-approach): adds a safety buffer
      ``BOX_ENTRY_MARGIN + BOX_HEIGHT + calibrated_tcp_z_offset + 0.10``
    ...
    * ``raw_z=True`` (pose move): the requested z is sent to IK
    ...
    if raw_z:
        ...
        target_z = z + BOX_ENTRY_MARGIN + BOX_HEIGHT + STATE.calibrated_tcp_z_offset + 0.10
...
def _pick_z_positions(z, grasp_lift_override=None):
    ...
    """
    # Use the calibrated Z offset — seeded from live prim norm at IK init,
    # then updated from the hover measurement every pick.  No fixed constants.
    tcp_z_offset = STATE.calibrated_tcp_z_offset
    print(f"  [TCP_Z] calibrated Z={tcp_z_offset:.4f} m  "
          f"XY=({STATE.calibrated_tcp_x_offset:.4f}, "
          f"{STATE.calibrated_tcp_y_offset:.4f}) m  "
          f"(frame='{STATE.target_frame}')")
...
async def _pick_descend(params):
    ...
    try:
        ...
        safe_z = zp["safe_z"]
        grasp_target_z = zp["grasp_target_z"]   # updated with live offset after hover
        # Use calibrated XY offsets from the most recent hover measurement.
        # First pick: seeds to 0 (no XY correction). From the second pick
        # onwards these hold the live-measured values so approach waypoints
        # are already close to correct without waiting for hover repose.
        ik_x = x + STATE.calibrated_tcp_x_offset
        ik_y = y + STATE.calibrated_tcp_y_offset

        safe_pos = np.array([ik_x, ik_y, safe_z])
        entry_pos = np.array([ik_x, ik_y, zp["entry_z"]])
        hover_pos = np.array([ik_x, ik_y, zp["hover_z"]])
        grasp_pos = np.array([ik_x, ik_y, grasp_target_z])
        ...
        current_ee = get_world_pos(EE_PATH)
        xy_error = np.linalg.norm(current_ee[:2] - np.array([ik_x, ik_y]))
        if xy_error > 0.002:  # >2mm drift → correct
            correct_pos = np.array([ik_x, ik_y, float(current_ee[2])])
            print(f"  [DESCEND] XY correction: drift={xy_error:.4f}m, "
                  f"snapping EE to ({ik_x:.3f}, {ik_y:.3f}, {current_ee[2]:.3f})")
        ...
        # 6a. Live TCP→EE offset: arm is now in DOWNWARD_ORIENTATION so
        #     this gives the reliable 3-axis offset for the actual grasp.
        #     Corrects for the startup-pose measurement artifact where the
        #     arm was not pointing down (tcp_ee_offset[2] ≈ 0 at init).
        try:
            _h_ee  = get_world_pos(EE_PATH)
            _h_tcp = get_world_pos(GRIPPER_TCP_PRIM)
            _live  = _h_ee - _h_tcp
            # Z: the TCP must be BELOW ee_link (positive offset in Z-up world).
            # If < 50 mm the prim may be misplaced — keep the last good value.
            _live_z = float(_live[2]) if float(_live[2]) > 0.05 else STATE.calibrated_tcp_z_offset
            ik_x = x + float(_live[0])
            ik_y = y + float(_live[1])
            grasp_target_z = (z + zp["grasp_lift"]) + _live_z
            # Persist all three axes so the NEXT pick's approach waypoints
            # use the fully dynamic live values instead of the seed constants.
            STATE.calibrated_tcp_x_offset = float(_live[0])
            STATE.calibrated_tcp_y_offset = float(_live[1])
            STATE.calibrated_tcp_z_offset = _live_z
            print(f"  [PICK] Hover TCP offset (live): "
                  f"dx={float(_live[0]):.4f}  dy={float(_live[1]):.4f}  "
                  f"dz={_live_z:.4f}  → calibrated and stored")
            print(f"  [PICK] Grasp target (IK frame): "
                  f"({ik_x:.4f}, {ik_y:.4f}, {grasp_target_z:.4f})")
        except Exception as _hoff_e:
            print(f"  [PICK] WARN: hover TCP read failed ({_hoff_e}); "
                  f"grasp_target_z={grasp_target_z:.4f} unchanged")
        steps.append({"phase": "hover_tcp_offset", "status": "ok"})
        ...
        hover_xy_err = np.linalg.norm(hover_ee[:2] - np.array([ik_x, ik_y]))
        ...
        if hover_xy_err > 0.002:  # >2mm → re-solve at hover height
            repose_target = np.array([ik_x, ik_y, float(hover_ee[2])])
            print(f"  [DESCEND] Re-posing EE to ({ik_x:.3f}, {ik_y:.3f}, {hover_ee[2]:.3f})")
        ...
        contact_lift_back = float(
            params.get("descent_contact_lift_back", 0.005))
        ...
        dx_drift = float(actual_ee[0]) - ik_x
        dy_drift = float(actual_ee[1]) - ik_y
        dz_drift = float(actual_ee[2]) - grasp_target_z
        ...
        if do_post_snap and xy_drift > snap_threshold:
            print(f"  [DESCEND] Post-descent XY snap: drift "
                  ...
                  f"({ik_x:.4f}, {ik_y:.4f}) at z={float(actual_ee[2]):.4f}")
            snap_pos = np.array([ik_x, ik_y, float(actual_ee[2])])
            ...
            if snap_ok:
                ...
                snap_residual = (
                    (float(snapped_ee[0]) - ik_x) ** 2
                    + (float(snapped_ee[1]) - ik_y) ** 2) ** 0.5
...
async def _place_object_ik(params):
    ...
    try:
        ...
        # XY: the TCP prim is close enough to directly above the tray target
        # for place; no additional XY offset correction is applied here.
        dest_x = x
        dest_y = y
        dest_top_z = z
...
async def start_bridge():
    ...
    try:
        ...
        # Measure full 3D TCP→EE world offset from live prims.
        # The arm may not be in DOWNWARD_ORIENTATION at init, so the Z
        # component alone is unreliable — use the 3D norm as the initial
        # Z seed (orientation-agnostic physical gripper length).
        # The first hover replaces this with the accurate in-pose Z value.
        try:
            _ee_p = get_world_pos(EE_PATH)
            _tcp_p = get_world_pos(GRIPPER_TCP_PRIM)
            STATE.tcp_ee_offset = _ee_p - _tcp_p
            _init_mag = float(np.linalg.norm(STATE.tcp_ee_offset))
            if _init_mag > 0.05:
                STATE.calibrated_tcp_z_offset = _init_mag
            print(f"[OK] TCP→EE world offset measured: "
                  f"dx={STATE.tcp_ee_offset[0]:.4f}, "
                  f"dy={STATE.tcp_ee_offset[1]:.4f}, "
                  f"dz={STATE.tcp_ee_offset[2]:.4f} m  "
                  f"(Z seed from norm={STATE.calibrated_tcp_z_offset:.4f} m)")
        except Exception as _tcp_err:
            print(f"[WARN] TCP offset measurement failed ({_tcp_err}); calibrated offsets not seeded")
